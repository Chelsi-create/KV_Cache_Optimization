...
output_path = Path("results/cache/wikimqa-13B_compatible.json")
...
for ex_idx, ex in enumerate(eval_dataset):
    ...
    results["blend"].append({
        ...
        "latency": blend_latency, "throughput": blend_throughput
    })
    results["full"].append({
        "answer": full_res, "ttft": full_ttft, "tpot": full_tpot,
        "latency": full_latency, "throughput": full_throughput
    })

# 计算平均值并保存
summary = {}
for mode in ["blend", "full"]:
    metrics = {k: [r[k] for r in results[mode]] for k in ["ttft", "tpot", "latency", "throughput"]}
